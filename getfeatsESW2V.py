...
def getelasticfeatures(q,a,q_lemma,a_lemma,es):
	...
	for i in range(len(a)):  # get the score of each combination of the answer and question
		...
		sc=0
		s = es.search(index='quizlets_lemma', _source=False, size=10, q=query2)
		...
		s = es.search(index='quizlets_lemma', _source=True, size=10, q=query2)
...
def genfeat_train(es):
	...
	pickle.dump([X,y,X_tuples,y_tuples],open('trainelasticw2v.pick','wb+'))
...
def genfeat_sub(es):
	...
	model2 = gensim.models.Word2Vec.load('model/word2vec_myck12_quizlet3_stem_23gram.model')
	vocab=clustervocab(model2.vocab)
	...
	np.save('subelasticw2v.npy',np.array(featureset))
